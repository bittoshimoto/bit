...
def parse_config_into_bit_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Bit Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bit_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bit_config.h'), "w", encoding="utf8") as bit_config:
        bit_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Bit-core msbuild configuration initialiser.')
    ...
    parse_config_into_bit_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bit_config.h'), os.path.join(SOURCE_DIR, 'config/bit-config.h'))
