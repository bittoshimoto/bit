...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} BIT_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Bit Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Bit Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example bit.conf file. if contrib/devtools/gen-bit-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/bit.conf" "${DISTNAME}/"
