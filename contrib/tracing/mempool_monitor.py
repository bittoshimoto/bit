...
""" Example logging Bit Core mempool events using the mempool:added,
...
def main(bitd_path):
    bitd_with_usdts = USDT(path=str(bitd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bitd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bitd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bitd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bitd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bitd_with_usdts])
    ...
    def parse_event(event):
        ...
        if type_ == "added":
            return (
                ...
                f" with feerate {data.fee/data.vsize:.2f} tos/vB"
                f" ({data.fee} tos, {data.vsize} vbytes)"
            )
        ...
        if type_ == "replaced":
            return (
                ...
                f" with feerate {data.replaced_fee/data.replaced_vsize:.2f} tos/vB"
                f" received {ts_dt.timestamp()-data.replaced_entry_time:.1f} seconds ago"
                f" ({data.replaced_fee} tos, {data.replaced_vsize} vbytes)"
                f" with {bytes(data.replacement_hash)[::-1].hex()}"
                f" with feerate {data.replacement_fee/data.replacement_vsize:.2f} tos/vB"
                f" ({data.replacement_fee} tos, {data.replacement_vsize} vbytes)"
            )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bitd")
        exit(1)
