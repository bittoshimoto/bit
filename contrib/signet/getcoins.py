...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bit-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bit-cli', help='bit-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bit address to which the faucet should send')
...
parser.add_argument('bit_cli_args', nargs='*', help='Arguments to pass on to bit-cli (default: -signet)')

args = parser.parse_args()

if args.bit_cli_args == []:
    args.bit_cli_args = ['-signet']


def bit_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bit_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bit_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bit_cli(['getnewaddress', 'faucet', 'bech32'])
