...
void ResetArgs(ArgsManager& local_args, const std::string& strArg)
{
    ...
    vecArg.insert(vecArg.begin(), "testbit");
...
BOOST_AUTO_TEST_CASE(patharg)
{
    ...
    const fs::path absolute_path{"/home/user/.bit"};
    ResetArgs(local_args, "-dir=/home/user/.bit");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);

    ResetArgs(local_args, "-dir=/root/../home/user/.bit");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);
    ...
    const fs::path relative_path{"user/.bit"};
    ResetArgs(local_args, "-dir=user/.bit");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);

    ResetArgs(local_args, "-dir=somewhere/../user/.bit");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);
    ...
    // line arguments or bit.conf values. Currently the -dir= case cannot be
