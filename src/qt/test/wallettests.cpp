                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<BitAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = BitUnits::formatWithUnit(unit, expected_balance, false, BitUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_bit-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_bit-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_bit-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    BitAmountField* amountInput = receiveCoinsDialog.findChild<BitAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_bit-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
