...
#include <qt/bitaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new BitAddressEntryValidator(parent));
    widget->setCheckValidator(new BitAddressCheckValidator(parent));
}
...
bool parseBitURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bit: URI
...
bool parseBitURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitURI(uriInstance, out);
}

QString formatBitURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bit:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBitConf()
{
    ...
    /* Open bit.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Bit (%s).lnk", chain));
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("bit-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bit.desktop file to the autostart directory:
