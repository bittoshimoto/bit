...
const std::string BitGUI::DEFAULT_UIPLATFORM =
...
BitGUI::BitGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitGUI::showModalOverlay);
...
void BitGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &BitGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Bit Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Bit Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bit: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bit command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BitGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &BitGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BitGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BitGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitGUI::showDebugWindow);
}

void BitGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void BitGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitGUI::setCurrentWalletBySelectorIndex);
...
void BitGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void BitGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitGUI::removeWallet);
...
WalletController* BitGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitGUI::incomingTransaction);
    connect(this, &BitGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BitGUI::removeWallet(WalletModel* walletModel)
{
...
void BitGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BitGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitGUI::toggleHidden);
...
void BitGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitGUI::openClicked()
{
...
void BitGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bit network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitGUI::updateHeadersSyncProgressLabel()
{
...
void BitGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BitGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitGUI::quitRequested);
...
void BitGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitGUI::show);
                e->ignore();
            }
        }
    }
...
void BitGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitGUI::incomingTransaction(const QString& date, BitUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitUnits::formatWithUnit(unit, amount, true));
...
void BitGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitGUI::detectShutdown()
{
...
void BitGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitGUI::subscribeToCoreSignals()
{
...
void BitGUI::unsubscribeFromCoreSignals()
{
...
bool BitGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitUnit> units = BitUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitUnit u : BitUnits::availableUnits()) {
        menu->addAction(BitUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitUnit newUnits)
{
    setText(BitUnits::longName(newUnits));
}
