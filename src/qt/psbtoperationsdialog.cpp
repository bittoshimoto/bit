...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = BitUnits::format(m_client_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(BitUnits::formatWithUnit(BitUnit::BIT, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(BitUnits::formatWithUnit(BitUnit::BIT, *analysis.fee));

        // add total amount in all subdivision units
        tx_description.append("<hr />");
        QStringList alternativeUnits;
        for (const BitUnits::Unit u : BitUnits::availableUnits())
        {
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(BitUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
