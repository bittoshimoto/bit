...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitUnit>("BitUnit");
#else
    qRegisterMetaType<BitUnit>("BitUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bit_de.qm (shortcut "de" needs to be defined in bit.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bit_de_DE.qm (shortcut "de_DE" needs to be defined in bit.qrc)
...
static const char* qt_argv = "bit-qt";
...
void BitApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitApplication::createOptionsModel(bool resetSettings)
{
...
void BitApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitGUI::quitRequested, this, &BitApplication::requestShutdown);
...
void BitApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitApplication::handleRunawayException);
    connect(this, &BitApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitApplication::parameterSetup()
{
...
void BitApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitApplication::requestInitialize()
{
...
void BitApplication::requestShutdown()
{
...
void BitApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // bit: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitGUI::handlePaymentRequest);
            connect(window, &BitGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitApplication::handleRunawayException(const QString &message)
{
...
void BitApplication::handleNonFatalException(const QString& message)
{
...
WId BitApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse bit.conf, determine network, switch to network specific
    ...
    // bit: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
