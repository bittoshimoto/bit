...
/** Call node.updateRwSetting() with Bit 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bit 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bit/bit/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bit 22.x will not try to read these.
...
//! Convert enabled/size values to bit -prune setting.
...
//! Get pruning enabled value to show in GUI from bit -prune setting.
...
//! Get pruning size value to show in GUI from bit -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bit.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bit_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitUnit>() == m_display_bit_unit) return;
    m_display_bit_unit = new_unit.value<BitUnit>();
    QSettings settings;
    settings.setValue("DisplayBitUnit", QVariant::fromValue(m_display_bit_unit));
    Q_EMIT displayUnitChanged(m_display_bit_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bit/bit/pull/8273
    ...
    // (https://github.com/bit-core/gui/issues/567).
