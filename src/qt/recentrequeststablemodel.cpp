    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return BitUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, BitUnits::SeparatorStyle::NEVER);
            else
                return BitUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    return tr("Requested") +
           QLatin1String(" (") +
           BitUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) +
           QLatin1Char(')');
}
