...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        connect(ui->customFee, &BitAmountField::valueChanged, this, &SendCoinsDialog::coinControlUpdateLabels);
        ...
        if (model->wallet().hasExternalSigner()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed Bit Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
...
bool SendCoinsDialog::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    processSendCoinsReturn(prepareStatus,
        BitUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), m_current_transaction->getTransactionFee()));
    ...
    for (const SendCoinsRecipient &rcp : m_current_transaction->getRecipients())
    {
        ...
        QString amount = BitUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
    ...
    if (model->wallet().privateKeysDisabled() && !model->wallet().hasExternalSigner()) {
        ...
        question_string.append(tr("Please, review your transaction proposal. This will produce a Partially Signed Bit Transaction (PSBT) which you can save or copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else if (model->getOptionsModel()->getEnablePSBITontrols()) {
        /*: Text to inform a user attempting to create a transaction of their current options. At this stage,
        ...
        question_string.append(tr("Please, review your transaction. You can create and send this transaction or create a Partially Signed Bit Transaction (PSBT), which you can save or copy and then sign with, e.g., an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
    ...
    if(txFee > 0)
    {
        ...
        question_string.append(BitUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    for (const BitUnit u : BitUnits::availableUnits()) {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(BitUnits::formatHtmlWithUnit(u, totalAmount));
    }
    question_string.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
        .arg(BitUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::sendButtonClicked([[maybe_unused]] bool checked)
{
    ...
    const bool always_show_unsigned{model->getOptionsModel()->getEnablePSBITontrols()};
...
void SendCoinsDialog::setBalance(const interfaces::WalletBalances& balances)
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(BitUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(BitUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->wallet().getDefaultMaxTxFee()));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(tr("%1/kvB").arg(BitUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value())));
    }
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(tr("%1/kvB").arg(BitUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK())));
