...
const char * const BIT_CONF_FILENAME = "bit.conf";
const char * const BIT_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bit-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BIT_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bit
    // macOS: ~/Library/Application Support/Bit
    // Unix-like: ~/.bit
#ifdef WIN32
    // Windows
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BIT_CONF_FILENAME));
}
