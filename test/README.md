This directory contains integration tests that test bitd and its
...
bitd and bit-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bit-util, bit-tx, ...).
...
Before tests can be run locally, Bit Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the bitd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bitd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bitd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bitd processes are running.

On linux, the test framework will warn if there is another
bitd process running when the tests are started.

If there are zombie bitd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bitd processes running on the system, so should not be used if any non-test
bitd processes are being run.**
...
sure bitd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bitd `debug.log`s will all be dumped
...
`test_framework.log` and bitd `debug.log`s can be combined into a single
...
call methods that interact with the bitd nodes-under-test.

If further introspection of the bitd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bitd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bitd <pid>
```
