...
# TestP2PConn: A peer we use to send messages to bitd, and store responses.
...
class CompactBlocksTest(BitTestFramework):
    ...
    # This test actually causes bitd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that bitd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for bitd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # bitd will not send blocktxn responses for blocks whose height is
