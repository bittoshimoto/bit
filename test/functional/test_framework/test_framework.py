...
class BitTestMetaClass(type):
    """Metaclass for BitTestFramework.

    Ensures that any attempt to register a subclass of `BitTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitTestFramework(metaclass=BitTestMetaClass):
    """Base class for a bit test script.

    Individual bit test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bit-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bitd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitd" + config["environment"]["EXEEXT"],
        )
        fname_bitcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bit-cli" + config["environment"]["EXEEXT"],
        )
        fname_bitutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bit-util" + config["environment"]["EXEEXT"],
        )
        self.options.bitd = os.getenv("BITD", default=fname_bitd)
        self.options.bitcli = os.getenv("BITCLI", default=fname_bitcli)
        self.options.bitutil = os.getenv("BITUTIL", default=fname_bitutil)
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("BitRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitd', self.options.bitd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bit-cli', self.options.bitcli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bitd=self.options.bitd,
                    bit_cli=self.options.bitcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bit.conf
    ...
    def skip_if_no_bitd_tracepoints(self):
        """Skip the running test if bitd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitd_zmq(self):
        """Skip the running test if bitd has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bit-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bit-wallet has not been compiled")

    def skip_if_no_bit_util(self):
        """Skip the running test if bit-util has not been compiled."""
        if not self.is_bit_util_compiled():
            raise SkipTest("bit-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bit-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bit-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bit-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bit-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bit_util_compiled(self):
        """Checks whether bit-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BIT_UTIL")
