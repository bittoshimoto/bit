...
class SimulateTxTest(BitTestFramework):
    ...
    def run_test(self):
        ...
        bit_fee = Decimal(funding["fee"])

        # w0 sees fee + 5 bit decrease, w2 sees + 5 bit
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("5") + bit_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -5 - bit_fee + 4.9999       -5 - bit_fee
        #       w1                      0                               +4.9999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("5") - bit_fee + Decimal("4.9999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("5") - bit_fee)
        ...
        tx2 = funding["hex"]
        bit_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("10") + bit_fee2))
