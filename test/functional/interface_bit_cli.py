...
# The block reward of coinbaseoutput.nValue (50) BIT/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 BIT/block.
...
class TestBitCli(BitTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `bit-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and bit-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and bit-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BIT/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BIT/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 BIT) to the default wallet.
            ...
            # Test bit-cli -generate with -rpcwallet in multiwallet mode.
