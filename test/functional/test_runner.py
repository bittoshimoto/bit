...
`test/functional/test_framework/test_framework.py:BitTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_bit_cli.py --legacy-wallet',
    'interface_bit_cli.py --descriptors',
...
def main():
    ...
    enable_bitd = config["components"].getboolean("ENABLE_BITD")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if bitd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "bitd"], stdout=subprocess.DEVNULL).returncode == 0:
...
class RPCCoverage():
    ...
    commands per `bit-cli help` (`rpc_interface.txt`).
