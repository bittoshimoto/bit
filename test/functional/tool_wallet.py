...
class ToolWalletTest(BitTestFramework):
    ...
    def bit_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/bit-wallet' + self.config["environment"]["EXEEXT"]
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.bit_wallet_process(*args)
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your Bit, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `bit-wallet help` raises an error. Use `bit-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `bit-wallet -help` for valid methods.')
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/bit/bit/issues/7463
    ...
    def test_dump_createfromdump(self):
        ...
        assert_equal(dump_data['BIT_CORE_WALLET_DUMP'], '1')
        ...
        dump_data["BIT_CORE_WALLET_DUMP"] = "0"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of bit-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        ...
        dump_data["BIT_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of bit-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not os.path.isdir(os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload"))
        bad_magic_wallet_dump = os.path.join(self.nodes[0].datadir, "wallet-bad_magic.dump")
        del dump_data["BIT_CORE_WALLET_DUMP"]
        dump_data["not_the_right_magic"] = "1"
        self.write_dump(dump_data, bad_magic_wallet_dump, "not_the_right_magic")
        self.assert_raises_tool_error('Error: Dumpfile identifier record is incorrect. Got "not_the_right_magic", expected "BIT_CORE_WALLET_DUMP".', '-wallet=badload', '-dumpfile={}'.format(bad_magic_wallet_dump), 'createfromdump')
        assert not os.path.isdir(os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload"))
