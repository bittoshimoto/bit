...
"""Test bitd with different proxy configuration.

Test plan:
- Start bitd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bitd side:
...
class ProxyTest(BitTestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: bitd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
        ...
        if test_onion:
            addr = "pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion:20972"
