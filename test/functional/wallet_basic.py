...
class WalletTest(BitTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 BIT from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 bit in block rewards plus fees, but
        ...
        # Send 10 BIT with subtract fee from amount
        ...
        # Sendmany 10 BIT with subtract fee from amount
        ...
        self.log.info("Test sendmany with fee_rate param (explicit fee rate in tos/vB)")
        fee_rate_sat_vb = 2
        fee_rate_bit_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_bit_kvb = Decimal(fee_rate_bit_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(amounts={address: 10}, fee_rate=str(fee_rate_sat_vb))
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_bit_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_bit_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        assert_raises_rpc_error(-6, "Fee rate (0.999 tos/vB) is lower than the minimum fee rate setting (1.000 tos/vB)",
        ...
        msg = "Fee rate (0.000 tos/vB) is lower than the minimum fee rate setting (1.000 tos/vB)"
        ...
        for target, mode in product([-1, 0], ["bit/kb", "tos/b"]):
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Bit address
            assert_raises_rpc_error(-5, "Invalid Bit address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Bit address
            assert_raises_rpc_error(-5, "Invalid Bit address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Bit address or script", self.nodes[0].importaddress, "invalid")
            ...
            self.log.info("Test sendtoaddress with fee_rate param (explicit fee rate in tos/vB)")
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_bit_kvb))
