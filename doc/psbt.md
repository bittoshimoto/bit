# PSBT Howto for Bit Core

Since Bit Core 0.17, an RPC interface exists for Partially Signed Bit
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bit/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Bit transactions that are not fully signed
...
#### Multisig with multiple Bit Core instances
...
Bit Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bit-cli` is used.
...
Later, when *V* BIT has been received on *Amulti*, and Bob and Carol want to
