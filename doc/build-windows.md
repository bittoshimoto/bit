...
The options known to work for building Bit Core on Windows are:
...
Note that for WSL the Bit Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bit, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
