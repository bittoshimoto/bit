...
1. The directly conflicting transactions all signal replaceability explicitly. A transaction is
   ...
   *Rationale*: See [BIP125
   explanation](https://github.com/bit/bips/blob/master/bip-0125.mediawiki#motivation).
...
4. The additional fees (difference between absolute fee paid by the replacement transaction and the
   ...
   feerate is 1 toshi/vB and the replacement transaction is 500 virtual bytes total, then the
   replacement pays a fee at least 500 toshis higher than the sum of the original transactions.

   *Rationale*: Try to prevent DoS attacks where an attacker causes the network to repeatedly relay
   transactions each paying a tiny additional amount in fees, e.g. just 1 toshi.
...
* Opt-in full replace-by-fee (without inherited signaling) honoured in mempool and mining as of
  **v0.12.0** ([PR 6871](https://github.com/bit/bit/pull/6871)).

* [BIP125](https://github.com/bit/bips/blob/master/bip-0125.mediawiki) defined based on
  Bit Core implementation.
