# User-space, Statically Defined Tracing (USDT) for Bit Core

Bit Core includes statically defined tracepoints to allow for more
...
**Note**: Bit Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bit Core
...
Multiple tools can list the available tracepoints in a `bitd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bit Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bitd
stap net        outbound_message 0x0000000000107c05 /src/bitd
stap validation block_connected  0x00000000002fb10c /src/bitd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bit Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitd | grep NT_STAPSDT -A 4 -B 2
