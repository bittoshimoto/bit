# Fuzzing Bit Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Bit Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/bit/bit
$ cd bit/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/bit/bit/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Bit Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bit_report.pdf) includes an external overview of the status of Bit Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/bit/bit/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/bit/bit/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/bit/bit/tree/master/src/test/fuzz).
...
It is possible to specify `bitd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`bit-core/qa-assets`](https://github.com/bit-core/qa-assets) repo.

To fuzz `process_message` using the [`bit-core/qa-assets`](https://github.com/bit-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/bit-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`bit-core/qa-assets`](https://github.com/bit-core/qa-assets) repo.

Every single pull request submitted against the Bit Core repo is automatically tested against all inputs in the [`bit-core/qa-assets`](https://github.com/bit-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Bit Core more robust.
...
with certain assembly code from Bit Core's code. See [developer notes on sanitizers](https://github.com/bit/bit/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Bit Core using afl++

## Quickstart guide

To quickly get started fuzzing Bit Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Bit Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Bit Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Bit Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Bit
Core without having to write any custom fuzzing harness. The `bitd` server
...
```sh
$ mkdir bit-honggfuzz-p2p/
$ cd bit-honggfuzz-p2p/
$ git clone https://github.com/bit/bit
$ cd bit/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/bitd.cpp b/src/bitd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/bitd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Bit Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Bit Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/bit/bit/pull/22472) for more information.
...
Bit Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bit-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=bit-core).
...
because Bit is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_bit-core/latest).
