Bit Core version 0.14.0 is now available from:

  <https://bit.org/bin/bit-core-0.14.0/>
...
Bit Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/bit/bit/blob/master/doc/zmq.md) for
...
When Bit Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `bit-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`bit-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/bit-cli -named help command="help"
    src/bit-cli -named getblockhash height=0
    src/bit-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/bit-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://bit.org/en/alert/2016-11-01-alert-retirement) in Bit Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Bit Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/bit/bit/pull/8753)
...
- incremental relay fee used for calculating BIP 125 replacement and mempool limiting. (1000 toshis/kB)
- calculation of threshold for a dust output. (effectively 3 * 1000 toshis/kB)
- minimum fee rate of a package of transactions to be included in a block created by the mining code. If miners wish to set this minimum they can use the new `-blockmintxfee` option.  (defaults to 1000 toshis/kB)
...
- #8722 `bb843ad` bit-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (bitdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnombit)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnombit)
...
- #8830 `ef0801b` test: Add option to run bit-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to bit-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to bit-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and bit-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (bitdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_bit_fuzzy` (gmaxwell)
...
### Documentation
- #20971 `806b9e7` Clarify witness branches in transaction.h serialization (dcousens)
...
- #9865 `289204f` Change bit address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` bitd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow bit-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` bit-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to bit-tx (jnewbery)
