Bit Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/bit-core-0.18.0/>
...
`/Applications/Bit-Qt` (on Mac) or `bitd`/`bit-qt` (on
Linux).
...
recommended to use Bit Core on unsupported systems.
...
10.10. Additionally, Bit Core does not yet change appearance when
...
`contrib/init/bitd.service`, it has been changed to use
`/var/lib/bitd` as the data directory instead of
`~bit/.bit`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitd` will
exist has enough space (check using `df -h /var/lib/bitd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bit/bit/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:20971:20971` (this is an extra `:20971` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Bit Core
...
The systemd init file (`contrib/init/bitd.service`) has been changed
to use `/var/lib/bitd` as the data directory instead of
`~bit/.bit`. This change makes Bit Core more consistent with
...
`datadir` option in `/etc/bit/bit.conf` will have no effect.
...
- A new short [document](https://github.com/bit/bit/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bit/bit/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bit/bit/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bit/bit/blob/master/doc/bit-conf.md)
  about the `bit.conf` file describes how to use it to configure
  Bit Core.

- A new document introduces Bit Core's BIP174 [Partially-Signed
  Bit Transactions
  (PSBT)](https://github.com/bit/bit/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bit/bit/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bit-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bit Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bit/bit/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bit/bit/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bit Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bit Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bit/bit/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bit/bit/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitd with the `-deprecatedrpc=generate`
...
- A new `bit-wallet` tool is now distributed alongside Bit
...
This section describes planned changes to Bit Core that may affect
other Bit software and services.

- Since version 0.16.0, Bit Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bit Core 0.20 (expected about
  a year after 0.18), Bit Core will default to native segwit
  ...
  to bech32 addresses, and if the Bit Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bit Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bit Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bit Core
...
- A sub-project of Bit Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bit Core.  See their [project
  page](https://github.com/bit-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bit Core's own implementation, although entropy
  gathered by Bit Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bit Core a
...
- On macOS, Bit Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bit_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbit_server.a` (jnewbery)
...
- #13926 [Tools] bit-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitd (MarcoFalke)
...
- #14521 qt, docs: Fix `bit-qt -version` output formatting (hebasto)
...
- #11625 Add BitApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbit_server` (laanwj)
...
- #15154 Configure: bit-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bit-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Bit Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbit_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bit-util-test.py (isghe)
...
- #12775 Integration of property based testing into Bit Core (Christewart)
...
- #12246 Bugfix: Only run bit-tx tests when bit-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bit` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bit` (MarcoFalke)
...
- #14186 bit-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bit.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bit.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bit-conf.md (hebasto)
...
- #15354 Add missing `bit-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bit folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bit/).
