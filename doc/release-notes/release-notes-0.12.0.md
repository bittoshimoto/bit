Bit Core version 0.12.0 is now available from:

  <https://bit.org/bin/bit-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Bit Core or other software:
...
pre-0.12 versions of Bit Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Bit Core version 0.11 or
earlier.
...
[libsecp256k1](https://github.com/bit-core/secp256k1) instead of OpenSSL.
...
(https://github.com/bit/bips/blob/master/bip-0130.mediawiki)
...
Bit Core 0.12 will have a strict maximum size on the mempool. The
...
Bit Core 0.12 also introduces new default policy limits on the length and
...
Bit Core 0.12 nodes. Bit Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/bit/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Bit Core 0.12 does not yet have support for
...
In Bit Core 0.12, when mempool limit has been reached a higher minimum
...
`-mintxfee=<i>`, which defaults to 1000 toshis per kB.
...
notes of v0.11.0](https://github.com/bit/bit/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/bit/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/bit/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 220971 to a RPC server bound on localhost on port 30971 do:

    stunnel -d 220971 -r 127.0.0.1:30971 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitrpc>
        ProxyPass http://127.0.0.1:20971/
        ProxyPassReverse http://127.0.0.1:20971/
...
- #6214 `87406aa` [OSX] revert renaming of Bit-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new bit-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include bit-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix bit-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of bit-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override BITD and BITCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [bit-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libbitconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "BIT" (Ross Nicoll)
