...
Bit Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/bit-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Bit Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bit Core version 0.18.0.
...
- Testing or debugging of implementations of the Bit P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bit Core. Bit Core logs debug messages
...
- OpenSSL is no longer used by Bit Core.  (#17265)

- BIP70 support has been fully removed from Bit Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bit/bit/issues/16599),
  [PR #16702](https://github.com/bit/bit/pull/16702), and the
  `bitd help` for more information.  This option is experimental and
...
- All custom settings configured when Bit Core starts are now
...
- The "Start Bit Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Bit Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bit Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bit-core/HWI) for
  signing.  Future versions of Bit Core should support a GUI option
...
- Bit Core's automatically-generated source code documentation is
  now available at https://doxygen.bitcore.org.  (#17596)
...
- The `bit-cli` utility used with the `-getinfo` parameter now
...
- The `bit-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=20972` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `bitstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bit-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBIT_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bit` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBIT_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bit-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bit schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bit.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bit` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bit-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bit-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bit Core files (hebasto)
...
- #17119 Fix broken bit-cli examples (andrewtoth)
...
- #17503 Remove bitness from bit-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitcore.org (laanwj)
...
- #17648 Rename wallet-tool references to bit-wallet (hel-o)
...
- #18341 Replace remaining literal BIT with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bit-cli -getinfo (malevolent)
